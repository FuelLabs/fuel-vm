...
impl_instructions! {
    ...
    "Get blob size"
    0xbb BSIZ bsiz [dst: RegId blob_id_ptr: RegId]
    "Load blob as code"
    0xbc BLDC bldc [blob_id_ptr: RegId offset: RegId len: RegId]
    "Load blob as data"
    0xbd BLDD bldd [dst_ptr: RegId blob_id_ptr: RegId offset: RegId len: RegId]
}
    ...
    for byte in 0..u8::MAX {
        if let Ok(repr) = Opcode::try_from(byte) {
            let should_allow = match repr {
                ...
                | TIME | TR | TRO | ECAL | BSIZ | BLDC | BLDD => false,
                _ => true,
            };
