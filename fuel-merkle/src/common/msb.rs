...
pub trait Msb {
    ...
    fn common_prefix_count(&self, other: &Self) -> Option<usize>;
}
...
impl<const N: usize> Msb for [u8; N] {
    ...
    fn common_prefix_count(&self, other: &Self) -> Option<usize> {
        ...
        for (byte1, byte2) in self.iter().zip(other.iter()) {
            ...
            count = count.checked_add((byte1 ^ byte2).leading_zeros())?;
            if byte1 != byte2 {
                break
            }
        }
...
mod test {
    ...
    fn test_common_prefix_count_returns_count_of_common_bits_when_all_bits_match() {
        ...
        let common_prefix_count = lhs_bytes.common_prefix_count(&rhs_bytes).unwrap();
