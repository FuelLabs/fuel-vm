...
mod tests {
    ...
    fn partial_leaf_state_root() {
        ...
        let expected_root = {
            ...
            let sz = next_multiple::<8>(code_len);
    ...
    #[test]
    fn multi_leaf_state_root() {
        let mut rng = StdRng::seed_from_u64(0xF00D);
        // 3 full leaves and a partial 4th leaf with 4 bytes of data
        let code_len = 3 * LEAF_SIZE + 4;
        let mut code = alloc::vec![0u8; code_len];
        rng.fill_bytes(code.as_mut_slice());
        ...
        let expected_root = {
            ...
            // Push full leaves
            tree.push(&code[LEAF_SIZE * 0..LEAF_SIZE * 1]);
            tree.push(&code[LEAF_SIZE * 1..LEAF_SIZE * 2]);
            tree.push(&code[LEAF_SIZE * 2..LEAF_SIZE * 3]);

            // Push partial leaf with manual padding.
            // We start by generating an 8-byte array, where each byte is the
            // PADDING_BYTE by default. The leaf is generated by copying the 4
            // remaining data bytes into the start of this array.
            let mut padded_leaf = [PADDING_BYTE; 8];
            padded_leaf[0..4].clone_from_slice(&code[LEAF_SIZE * 3..LEAF_SIZE * 3 + 4]);
            tree.push(&padded_leaf);
            tree.root().into()
        };
