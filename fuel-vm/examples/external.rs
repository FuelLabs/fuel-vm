...
use std::{path::PathBuf, fs::{File, self}, io::{Seek, SeekFrom, Read}};

use fuel_asm::{PanicReason, RegId, op, GTFArgs};
use fuel_tx::{Script, TransactionBuilder, Finalizable, ConsensusParameters, Receipt};
use fuel_vm::{prelude::{Interpreter, MemoryRange, MemoryClient, IntoChecked}, storage::MemoryStorage};
...
fn main() {
    ...
    vm.set_ecal(|vm, a, b, c, d| {
        ...
        let _ = file.seek(SeekFrom::Start(a)).map_err(|_| PanicReason::EcalError)?;

        // Allocate the requested buffer in vm memory and read directly from the file into it
        vm.allocate(b)?;
        let r = MemoryRange::new(vm.registers()[RegId::HP], b)?;
        file.read(&mut vm.memory_mut()[r.usizes()]).map_err(|_| PanicReason::EcalError)?;
    ...
    let script = vec![
        ...
        op::gtf_args(0x22, 0x00, GTFArgs::ScriptData), // File path is script_data
