...
fn backtrace() {
    ...
    let contract_call = test_context.setup_contract(function_call, None, None).contract_id;
    ...
    let (_, backtrace) = test_context.execute_tx_with_backtrace(tx).expect("Should execute tx");
